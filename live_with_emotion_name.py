...
def prepare_input(audio, sample_rate):
...
def predict_emotion(audio, model):
    # Prepare input data
    sample_rate = 48000
    input_data = prepare_input(audio, sample_rate)
...
def real_time_emotion_recognition(model_path):
    ...
    def callback(indata, frames, time, status):
        if status:
            print(status)
        audio = indata[:, 0]
        predicted_emotion = predict_emotion(audio, model)
        print("Predicted Emotion:", predicted_emotion)

    # Start audio stream
    sample_rate = 48000  # Replace with your audio file's sample rate
    with sd.InputStream(callback=callback, channels=1, samplerate=sample_rate, blocksize=int(sample_rate * 3)):
        print("Listening... Press Ctrl+C to stop.")
        while True:
            sd.sleep(1000)  # Keep the stream alive; adjust as needed

# Example usage
if __name__ == "__main__":
    model_path = 'cnn_lstm_parallel_model.pt'
    try:
        real_time_emotion_recognition(model_path)
